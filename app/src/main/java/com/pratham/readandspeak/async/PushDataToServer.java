...
/*            JSONArray sessionArray = new JSONArray();
            ...
            for (Session session : newSessions) {
                ...
                if (!RASApplication.isTablet)
                    rootJson.put(RAS_Constants.STUDENTS, studentArray);
                rootJson.put(RAS_Constants.SESSION, sessionArray);
                rootJson.put(RAS_Constants.ATTENDANCE, attendanceArray);
                rootJson.put(RAS_Constants.SCORE, scoreArray);
                rootJson.put(RAS_Constants.LOGS, logArray);
                rootJson.put(RAS_Constants.ASSESSMENT, assessmentData);
                rootJson.put(RAS_Constants.SUPERVISOR, supervisorData);
                rootJson.put(RAS_Constants.LEARNTWORDS, learntWords);

            }*/
           /* List<com.pratham.readandspeak.domain.Status> metadata = AppDatabase.getDatabaseInstance(context).getStatusDao().getAllStatuses();
            for (com.pratham.readandspeak.domain.Status status : metadata) {
            ...
            metadataJson.put(RAS_Constants.SCORE_COUNT, (metadata.size() > 0) ? metadata.size() : 0);
            rootJson.put(RAS_Constants.METADATA, metadataJson);
*/
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        if (checkEmptyness(requestString))
        pushDataToServer(context, requestString, RASApplication.uploadDataUrl);
        return null;
    }
...
*/
    ...
    private void pushDataToServer(Context context, String data, String url) {
        try {
