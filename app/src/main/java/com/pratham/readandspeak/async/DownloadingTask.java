...
import static com.pratham.readandspeak.utilities.RAS_Constants.FILE_DOWNLOAD_STARTED;
...
//                long download_percentage_old = 00;
            ...
            if (input != null)
                input.close();
            if (folder_name.equalsIgnoreCase(RAS_Constants.GAME)) {
    ...
    private void downloadCompleted() {
        ...
        for (ContentTable d : temp) {
            ...
            d.setContentLanguage(FastSave.getInstance().getString(RAS_Constants.LANGUAGE, RAS_Constants.ENGLISH));
            d.isDownloaded= ""+true;
            d.setOnSDCard(false);
        }
    ...
    public static void downloadImage(String url, String filename) {
        File dir = new File(RASApplication.pradigiPath + "/.RAS/English/RAS_Thumbs"); //Creating an internal dir;
    ...
    protected void onProgressUpdate(Object... values) {
        ...
        eventMessage.setMessage(RAS_Constants.FILE_DOWNLOAD_UPDATE);
        EventBus.getDefault().post(eventMessage);
    }
    ...
    protected void onPostExecute(Object r) {
        ...
        if (result) {
            EventMessage eventMessage = new EventMessage();
            eventMessage.setMessage(RAS_Constants.FILE_DOWNLOAD_COMPLETE);
            EventBus.getDefault().post(eventMessage);
//            gamesDisplay.onDownloadCompleted(downloadID, content);
        } else {
            EventMessage eventMessage = new EventMessage();
            eventMessage.setMessage(RAS_Constants.FILE_DOWNLOAD_ERROR);
            EventBus.getDefault().post(eventMessage);
